...
mod tests {
    ...
    // ── PROP-02: Arena allocation invariants ──────────────────────────
    //
    // Property-based tests generated by proptest. Each test exercises
    // the Arena under arbitrary sequences of alloc/free operations,
    // verifying that core invariants hold for all generated inputs.
    //
    // Key invariants under test:
    //   - No double-allocation of live ObjectIds
    //   - Freed slots are reusable
    //   - No ID overlap among live allocations at any point
    //   - Alloc-get roundtrip is consistent
    //   - Free-then-get returns None
    //   - Live allocation count tracks correctly

    /// PROP-02: No double-allocation — all ObjectIds among N live
    /// allocations are distinct.
    proptest! {
        #[test]
        fn prop_arena_alloc_ids_unique(count in 1u32..=50u32) {
            let mut arena: Arena<TestObject> = make_arena();
            ...
            prop_assert_eq!(
                sorted.len(),
                original_len,
                "duplicate ObjectId detected among {} live allocations",
                original_len
            );
        }
    }

    /// PROP-02: Freed slots are reusable — after allocating and freeing N
    /// objects, N more allocations must succeed (slots returned to freelist).
    proptest! {
        #[test]
        fn prop_arena_alloc_free_realloc(count in 1u32..=20u32) {
            let mut arena: Arena<TestObject> = make_arena();
            let mut ids: Vec<ObjectId> = Vec::new();

            // Phase 1: allocate count objects, skipping on OOM.
            for _ in 0..count {
                match arena.allocate() {
                    Ok((id, _)) => ids.push(id),
                    Err(AllocError::OutOfMemory) => break,
            ...
            let allocated = ids.len();
            // Free them all.
            for id in ids {
                arena.free(id);
            }

            // Phase 2: re-allocate the same number — must all succeed
            // because the freed slots are back on the freelist.
            let mut realloc_count = 0usize;

            for _ in 0..allocated {
                match arena.allocate() {
                    Ok(_) => realloc_count += 1,
                    Err(AllocError::OutOfMemory) => break,
                }
            }
            ...
            prop_assert_eq!(
                realloc_count,
                allocated,
                "re-allocation count must equal free count (freed slots must be reusable)"
            );
        }
    }

    /// PROP-02: No ID overlap — at no point during a mixed alloc/free
    /// sequence do two live allocations share the same ObjectId.
    ///
    /// The operation sequence is encoded as Vec<bool>: true = alloc,
    /// false = free the most-recently-allocated live object (if any).
    proptest! {
        #[test]
        fn prop_arena_mixed_ops_no_overlap(ops in prop::collection::vec(any::<bool>(), 2..=80)) {
            let mut arena: Arena<TestObject> = make_arena();
            let mut live_ids: Vec<ObjectId> = Vec::new();

            for op in ops {
                if op {
                    // Alloc
                    match arena.allocate() {
                        Ok((id, _)) => live_ids.push(id),
                        Err(AllocError::OutOfMemory) => {}
                    }
                } else {
                    // Free most-recently-allocated live object (if any).
                ...
                prop_assert_eq!(
                    sorted.len(),
                    len,
                    "ID overlap detected among {} live allocations after operation",
                    len
                );
            }
        }
    ...
    /// PROP-02: Alloc-get roundtrip — every successful allocate() returns
    /// an ObjectId that retrieves the stored value via get().
    proptest! {
        #[test]
        fn prop_arena_alloc_get_roundtrip(count in 1u32..=30u32, offset in 0u32..=1000u32) {
            let mut arena: Arena<TestObject> = make_arena();
            let mut pairs: Vec<(ObjectId, u32)> = Vec::new();
            ...
            for i in 0..count {
                ...
                match arena.allocate() {
                    Ok((id, obj)) => {
                        ...
                        pairs.push((id, expected_value));
                    }
                    Err(AllocError::OutOfMemory) => break,
                }
            }

            // Every allocated id must retrieve its stored value.
    ...
    /// PROP-02: Free-get consistency — after free(id), get(id) returns None;
    /// all other live ids remain retrievable.
    proptest! {
        #[test]
        fn prop_arena_free_then_get_none(count in 1u32..=30u32, free_index_raw in 0u32..=29u32) {
            let mut arena: Arena<TestObject> = make_arena();
            let mut ids: Vec<ObjectId> = Vec::new();
            ...
            prop_assert!(
                arena.get(freed_id).is_none(),
                "get after free must return None for freed id {}",
                freed_id.0
            );

            // All other ids must still be live.
    ...
    /// PROP-02: Live count consistency — after a mixed alloc/free sequence,
    /// the number of ids that return Some from get() matches the expected
    /// live count derived from the operation sequence.
    proptest! {
        #[test]
        fn prop_arena_live_count_consistent(ops in prop::collection::vec(any::<bool>(), 2..=60)) {
            let mut arena: Arena<TestObject> = make_arena();
            let mut live_ids: Vec<ObjectId> = Vec::new();
            ...
            prop_assert_eq!(
                actual_live,
                expected_live,
                "live count mismatch: expected {} live ids, get() found {}",
                expected_live,
                actual_live
            );
        }
