...
| --- | -------------------------------------------------------------------------------------------------------- | --------------------------------------------- | -------------------------- |
| M1  | Badge size                                                                                               | D17 (64-bit default stated)                   | settled (D58)              |
| M2  | Scheduler callback signature                                                                             | D2, D42, D50, D53                             | settled (D59)              |
| M3  | Send-to-waiting-receiver optimization                                                                    | D13, D50 (probably no separate mechanism)     | resolved (subsumed by D50) |
| M4  | Page-addressed vs byte-addressed                                                                         | A5, D25 (kernel absorbs → byte with rounding) | settled (D60)              |
| M5  | Observer schema downstream (register save layout, budget encoding, default profile, self-reference caps) | D43, D39, ARM64 ABI                           | settled (D57)              |
| M6  | Fault message content and enqueue mechanism                                                              | D12, D18, D20, D28, D7                        | settled (D61)              |
| M7  | Pulsar creation API shape                                                                                | D32, D44, D35 (composable pattern)            | settled (D62)              |
| M8  | Pulsar message content layout                                                                            | D28, D44                                      | settled (D63)              |
| M9  | Badge-closure message format                                                                             | D17, D28                                      | settled (D64)              |
| M10 | Badge on kernel-created send-once caps                                                                   | D16, D17                                      | settled (D65)              |
| M11 | Clock access authority                                                                                   | D44 (CNTKCTL_EL1 per-Observer)                | settled (D66)              |
...
| --- | --------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| G1  | Revocation add-ons (generation vs CDT vs both)                                                | Speed vs granularity; cross-core prompt-effect                                                                                                                   | settled (D67)                          |
| G2  | Shared capability tables                                                                      | D8 per-Observer sufficiency vs sharing pressure                                                                                                                  | resolved (D8 confirmed, journal 065)   |
| G3  | Interrupt priority and routing                                                                | A5 (absorb) vs A3 (expose for RT workloads)                                                                                                                      | settled (absorbed by D22, journal 066) |
| G4  | Pager unavailability protocol                                                                 | Timeout/watchdog vs "let it hang" vs cleanup chain                                                                                                               | settled (D68)                          |
| G5  | Interrupt masking during fast path                                                            | Worst-case latency vs scheduling consistency                                                                                                                     | settled (D69)                          |
| G6  | Sub-page packing under D24                                                                    | Waste memory vs copy vs accept no cleanup                                                                                                                        | settled (D70)                          |
| G7  | Badge condition form                                                                          | Range vs bitmask vs predicate (IPC perf impact)                                                                                                                  | settled (D71)                          |
| G8  | Split-to-new vs split-to-existing                                                             | One syscall or two                                                                                                                                               | settled (D45 updated, journal 071)     |
| G9  | Pulsar: duration vs absolute deadline                                                         | API shape and drift compensation                                                                                                                                 | settled (D72)                          |
| G10 | Send-once exemption encoding                                                                  | Consumed-by-use vs closed-without-use                                                                                                                            | settled (D73)                          |
...
| G12 | Implementation hardening checklist (KASLR, guard pages, freed-memory zeroing, stack canaries) | Not derivable from design graph — leaf-node techniques that don't interact with the object model; track as a checklist, not derivations                          | deferred (not derivation-level)        |
