...
Frames bind to, or an emergent property of individual memory-object mappings?

**Answer:** The address space is a first-class, capability-designated kernel
object. Frames bind to an address space; multiple Frames can bind to the same
one, sharing its page table tree. The vocabulary's "Space" (resource claim /
memory budget) remains a separate concept. Working name: "address space" — final
naming deferred to public API.
...
- journal/005 (D5): "Address space structure per Frame" — one level down.
- journal/006 (D6): "Frame-Space binding model" — unsettled.
- journal/008 (D8): "Frame-Space binding and table sharing" — deferred, with D8
  explicitly conditioning capability table sharing on this question's outcome.
...
Frame attribute without a separate object). The migrating-threads alternative
(Spring, Composite, L4 LRPC) still uses first-class address spaces — the
execution unit migrates between them.

No specific research document on the Frame-Space binding question existed prior
to this exploration.
...
translations the MMU uses) belong to the Frame, or does the Frame point to it?

**Option A — First-class address space:** The page table tree is a
capability-designated kernel object. Frames hold capabilities to address spaces
and bind to one. Multiple Frames can bind to the same address space, sharing the
tree. Memory objects (D9) are mapped into the address space, not into the Frame.

**Option B — Emergent (address space = Frame attribute):** Each Frame owns its
page tables directly. Memory objects are mapped into a Frame. "Sharing" means
multiple Frames independently mapping the same memory objects. No structural
sharing of page tables.

### Path 1: A5 — mapping consistency is interface complexity

Under the emergent model, maintaining mapping consistency across Frames that
share memory is pushed to userspace. Adding a shared mapping to N Frames
...
makes the mapping visible to all bound Frames. The kernel absorbs the
consistency concern.
...
under the emergent model must: track which Frames are in the group, replicate
all mappings when a Frame joins, propagate new mappings to all group members,
and handle partial failures. This is a userspace address space manager — the
kernel object reimplemented outside the kernel, which is precisely the A5
antipattern.

### Path 2: D1 — hot-path TLB cost

Context switching between Frames that share memory but have independent page
tables (emergent model) requires a TTBR0 reload on every switch — different page
table roots, different ASIDs. Under the first-class model, Frames bound to the
same address space share a TTBR0 value and ASID. Context switch between them
skips the TTBR write entirely.

The direct TTBR switch cost is small (~13 cycles, pipeline flush on ARM64
Cortex-A76). The real cost is **TLB capacity pressure**. With per-Frame ASIDs
(emergent model), co-located Frames that share all mappings each need their own
TLB entries — the same virtual→physical translations duplicated under different
ASIDs. On a Cortex-A76 with 1280-entry L2 TLB, 10 Frames with a 200-page shared
working set need 2000 TLB entries (emergent) vs. 200 entries (first-class). The
overflow causes page table walks at ~30 cycles per miss (to L2 cache) or ~290+
cycles (to DRAM).

This matters for high-concurrency workloads (many-worker servers, databases with
thread pools). For workloads with few Frames, small working sets, or infrequent
switching, the difference is negligible.

Kernel-internal optimizations could partially mitigate this (shared ASIDs for
Frames with identical mappings), but such optimizations amount to the kernel
internally re-deriving the first-class address space concept — tracking "which
Frames share an address space" as an internal bookkeeping structure. The
...
independently of Frame access (suspend/destroy rights). A supervisor can grant a
loader the ability to map code into an address space without granting it control
over any Frame bound to that address space.

Under the emergent model, the address space is inseparable from the Frame. To
grant someone the ability to modify a Frame's mappings, you must grant them a
capability to the Frame itself — conflating mapping authority with execution
authority.

### Supporting observations

**D6's language.** D6 says "one address space binding" — not "one address
space." The word "binding" implies something the Frame is bound TO, not
something it inherently IS. D6 also says "process is a userspace convention (a
group of Frames sharing a Space)" — "sharing" reads as multiple Frames bound to
the same object, not independently replicating mappings.
...
**Vocabulary cardinality.** The vocabulary says "A Frame correlates one or more
Spaces." If Space (vocabulary) were the address space, this would contradict D6
("one address space binding"). Space-as-budget (one or more memory claims per
Frame) is consistent with both the vocabulary and D6. This confirms that Space
(vocabulary) and the address space are distinct concepts.
...
Frames) out of POSIX familiarity, even though D4's per-Frame authority model
suggests selective sharing is more principled.

This concern is real but addressable at the API level:

1. **No default address space.** Frame creation requires an explicit address
...
2. **Equal friction.** Creating a new address space is one syscall — the same
   ...
   of least resistance.
3. **Capability-gated.** Binding a Frame to an existing address space requires
   holding a capability with bind rights. Sharing cannot happen accidentally.
4. **Per-Frame authority preserved.** Even Frames sharing an address space have
   separate capability tables (D8). Kernel-level authority is never shared by
   default.
...
address-space concept. The "one or more Spaces" cardinality in the Frame
vocabulary entry is consistent with Space-as-budget (a Frame can hold multiple
memory resource claims) and inconsistent with Space-as-address-space (a Frame
has exactly one address space binding, per D6).
...
The emergent model (address space as a Frame attribute) was rejected on three
independent paths:

| Path | Argument                                                                                  |
| ---- | ----------------------------------------------------------------------------------------- |
| A5   | Mapping consistency for shared address spaces is essential complexity pushed to userspace |
| D1   | TLB capacity pressure from per-Frame ASIDs on co-located workloads                        |
| D4   | Cannot delegate address-space access independently of Frame access                        |
...
question, natural per-Frame budget — were examined and found to be largely
...
- **Binding mutability.** Can a Frame rebind to a different address space after
  creation? D6 says "one address space binding" but does not say it is
  immutable.
- **Address space lifecycle.** When is an address space destroyed? When the last
  capability is dropped? When the last Frame unbinds?
- **Frame creation API.** Whether binding is a parameter of Frame creation or a
  separate operation. The "no default, equal friction" API guidance above is a
  design intent, not a settled interface.
- **Capability table sharing.** D8 deferred this to the Frame-Space binding
  model. Now that same-address-space Frame groups are a first-class concept,
...
isolated-address-space pattern (single-Frame isolation) are expressible under
...
- A downstream derivation (capability table sharing, Frame lifecycle) reveals
  that first-class address spaces force essential complexity into userspace that
  per-Frame address spaces would have avoided
