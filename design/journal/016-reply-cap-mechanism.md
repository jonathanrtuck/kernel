# 016 — Reply via pre-allocated reply endpoint with send-once cap

**Date:** 2026-04-18 **Starting point:** D15 settled unidirectional,
many-to-many endpoints and listed reply-cap mechanism as the first downstream
question: "One-shot (kernel mints during call, auto-revoked after reply) vs.
persistent (client creates reply endpoint, transfers send cap explicitly)."

---

## The question

How does RPC reply routing work over unidirectional endpoints? D15 accepted
...
load-bearing for choosing Option A (client's reply endpoint) — it made the
sender's interface identical for both cases.
...
the fault message, then calls resume(). IPC reply is send-to-endpoint; fault
resume is a typed kernel op. They are different mechanism families per D7. The
...
### Option 1: Persistent reply endpoint (archive's choice)

Client creates a regular endpoint, holds receive cap, transfers a send cap in
every RPC request. Server sends reply to the transferred cap. Uses only existing
mechanisms.

- No new kernel types (five types unchanged)
- Standard D8 cap table entries, standard D11 revocation
- Costs: per-client reply endpoint, per-RPC cap transfer, client manages
  lifecycle, reply path goes through endpoint queue, server can retain send cap
  after reply (capability leak vector)
...
naming the blocked caller directly (not an endpoint). Server sends reply via the
Reply cap; cap is consumed.

- Reply path bypasses endpoint queue (names caller directly)
...
### Option 3: Pre-allocated reply endpoint with send-once cap

Each Observer has a pre-allocated reply endpoint (a regular endpoint). D8's
rights mask gains a send-once right: use-limited attenuation that applies to any
endpoint cap — consumed after one send. On Call(), the kernel creates a
send-once cap to the caller's reply endpoint, includes it in the request
message, and blocks the caller on the reply endpoint. Server sends reply to the
send-once cap; cap is consumed.

- No new kernel types (reply endpoint is a regular endpoint)
...
- Kernel can optimize the reply fast path internally (bypass queue for known
  reply-endpoint patterns — implementation detail, not exposed in object model)
...
**1. The reply endpoint IS an endpoint.** A dedicated Reply type (Option 2)
names the blocked caller directly, bypassing the endpoint queue. But the
fast-path bypass is an optimization, not a categorical difference. An endpoint
with exactly one waiter and a send-once cap is structurally isomorphic to a
Reply cap — the kernel knows the caller is waiting (it set up the Call()) and
can optimize the reply path behind the endpoint interface. The optimization is
real; the new type is not structurally required.
...
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| One-shot Reply type (Option 2) | New kernel type not structurally required — an endpoint with send-once achieves the same function; fast-path bypass is an optimization behind the interface |
...
| Synchronous MsgSend/MsgReply   | Foreclosed by D13 — synchronous-only contradicts queued model; caller can block on reply endpoint voluntarily to achieve the same effect                    |

---

## Archive convergence

The archive (journal/011) chose Option A: client's reply endpoint with a send
cap transferred in the request. Option 3 is a refinement of the same approach:
same object model (reply endpoint is a regular endpoint), but with send-once
attenuation on the transferred cap. The archive did not have send-once; the
client transferred a regular send cap, which the server could retain after
reply. Send-once addresses this: the cap is consumed on use, preventing
post-reply retention.
...
on a standard capability to a standard endpoint, not a new mechanism. The
one-shot behavior composes from the existing capability + endpoint model with
...
**Pre-allocated reply endpoint with send-once cap (Option 3).**

Each Observer has a pre-allocated reply endpoint — a regular endpoint (D15). The
D8 rights mask gains a **send-once** right: a use-limited attenuation where the
capability is consumed after one send operation. On Call(), the kernel creates a
send-once cap to the caller's reply endpoint, includes it in the request
message, and blocks the caller on its reply endpoint. The server sends the reply
to the send-once cap; the cap is consumed. The reply endpoint persists
(pre-allocated, reused across RPCs); the cap is ephemeral.
...
- **Call() and ReplyRecv() syscall details.** Part of the specific syscall
  ...
  endpoint); ReplyRecv() (atomic reply + wait for next request) is a natural
  companion. Exact signatures and flags are deferred.
- **Reply endpoint per Observer policy.** Pre-allocated at Observer creation, or
...
- **Send-once right encoding.** How send-once is represented in D8's rights mask
  ...
  entry layout.
- **Shared reply endpoint with badge disambiguation.** An Observer could use one
  reply endpoint for all servers, with badges distinguishing which server's
  reply arrived. Requires badge semantics (unsettled). Alternative: one reply
  endpoint per Observer (simpler, no disambiguation needed).
